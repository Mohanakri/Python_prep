...
## 🔁 Complex Operator Combinations (with output and comments)

| Expression | Description | Output |
|------------|-------------|--------|
| `3 + 4 * 2` | Multiplication before addition | `11` |
| `(3 + 4) * 2` | Parentheses change priority | `14` |
| `10 > 5 and 2 < 3` | Both comparisons true | `True` |
| `10 > 5 and 2 > 5` | Second condition false | `False` |
| `not 3 > 2` | NOT applied after comparison | `False` |
| `2 ** 3 * 2` | Power before multiplication | `16` |
| `True or False and False` | AND before OR | `True` |
| `not True or False` | NOT first, then OR | `False` |
| `5 + 3 * 2 > 10 or 2 == 2` | Mixed with OR | `True` |
| `not (5 > 3 and 2 < 1)` | Inner evaluated first | `True` |
| `'a' in 'cat' and 1 in [1,2,3]` | Membership and logical | `True` |
| `5 == 5 is True` | `==` then `is` | `True` |
| `(2 + 3) * (4 - 1) // 2` | Grouped math | `7` |
| `x := 4 + 2 > 5` | Walrus assigns before compare | `True` |
| `(x := 2 * 3) < 10` | Assign 6 to x and compare | `True` |

---

## 📘 Tips

- Always use `()` to control **complex logic** and **make code readable**
- `and` has higher precedence than `or`
- `not` has higher precedence than both `and` and `or`
- Assignment (`=`, `+=`, etc.) has **lowest** precedence (evaluated last)
